...
public class ClientHandler extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, ByteBuf buf) throws Exception {
        ...
        while (packageData != null && packageData.length > 16) {
            int newPackageLength = ConfigService.byteArrayToInt(packageData, 0);
            if (newPackageLength <= 0 || newPackageLength > 8000000) {
                logger.info("包长度不合法" + newPackageLength);
                logger.info(Arrays.toString(bys));
                packageData = null;
...
//                logger.debug(Arrays.toString(bys));
                return;
            }
            scheduledExec.submit(() -> {
                try {
                    int seq = ConfigService.byteArrayToInt(bys, 12);     // 12 - 16 seq标识
                    callBack.msgCallBack(seq, bys);
                } catch (Exception e) {
                    logger.error("handlePackage:scheduledExec", e);
                }
            });
        }
    }
